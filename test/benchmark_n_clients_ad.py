...
def main(args: argparse.Namespace):
    ...
    if not args.use_existing_dump:
        ...
        if args.save_name:
            result_dump_filename = args.save_name
            if not result_dump_filename.endswith('.json'):
                result_dump_filename += '.json'
        data = dict(args=vars(args), results=[dataclasses.asdict(r) for r in RESULTS], benchmark_time=benchmark_time)
        ...
        with open(args.use_existing_dump) as f:
            ...
            benchmark_time = previous_data['benchmark_time']
...
def wait_service(addr, port):
    import requests
    url = f'http://{addr}:{port}/'
    print(f"waiting for the service {url} ...")
    while True:
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_name", default=None, help="don't run the benchmark, use the existed benckmark dump from previous runs")
