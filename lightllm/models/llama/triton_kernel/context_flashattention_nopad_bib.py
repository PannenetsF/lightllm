...
def _fwd_kernel(
    ...
    off_q = (cur_q_start + offs_m[:, None]) * stride_qbs + cur_head * stride_qh + offs_d[None,
                                                                                  :] * stride_qd
    ...
    off_o = (cur_q_start + offs_m[:, None]) * stride_obs + cur_head * stride_oh + offs_d[None,
                                                                                  :] * stride_od
    out_ptrs = Out + off_o
    # tl.device_print("o write ", cur_batch, cur_head, start_m, cur_q_start)
    tl.store(out_ptrs, acc, mask=offs_m[:, None] < cur_batch_seq_len)
    return


@torch.no_grad()
def context_attention_fwd_bib(q, k, v, o, b_start_loc, b_seq_len, b_q_start_loc, max_input_len):
