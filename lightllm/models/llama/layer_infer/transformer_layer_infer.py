...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def _bind_attention(self):
        ...
        if "ppl_int8kv" in self.mode:
            ...
            self._token_attention_kernel = partial(
                LlamaTransformerLayerInfer._token_decode_bib, self)
            self._copy_kv_to_mem_cache = partial(
                LlamaTransformerLayerInfer._copy_kv_to_mem_cache_normal, self)
        elif "bib_decoding_cuda" in self.mode:
            # find bib_decoding_cuda=xxx
            chunk_size = 128
            for item in self.mode:
                if "bib_decoding_cuda=" in item:
                    chunk_size = int(item.split("=")[-1])
                    break
            self._bib_chunk_size = chunk_size
            self._token_attention_kernel = partial(LlamaTransformerLayerInfer._token_decode_bib_cuda, self)
            self._copy_kv_to_mem_cache = partial(LlamaTransformerLayerInfer._copy_kv_to_mem_cache_normal, self)
        else:
    ...
    def _token_decode_bib(self, q, infer_state: LlamaInferStateInfo, layer_weight, out=None, bib_chunk_size=128):
    ...
    def _token_decode_bib_cuda(self, q, infer_state: LlamaInferStateInfo, layer_weight, out=None):
        from lightllm.models.llama.triton_kernel.token_attention_bib_cuda import token_attention_bib_cuda
        cache_k = infer_state.mem_manager.key_buffer[self.layer_num_]
        cache_v = infer_state.mem_manager.value_buffer[self.layer_num_]
        batch = q.shape[0]
        return token_attention_bib_cuda(
            q.view(batch, self.tp_q_head_num_, -1), cache_k, cache_v, out, infer_state, self._bib_chunk_size
        )
