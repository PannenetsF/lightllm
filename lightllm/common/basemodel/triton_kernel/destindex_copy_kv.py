...
@triton.jit
def _fwd_kernel_srcindex_copy_kv(
    K, src_loc,
    Out,
    ...
    src_index = tl.load(src_loc + cur_index)

    k_ptrs = K + src_index * stride_k_bs + stride_k_h * offs_h[:, None] + stride_k_d * offs_d[None, :]
    o_ptrs = Out + cur_index * stride_o_bs + stride_o_h * offs_h[:, None] + stride_o_d * offs_d[None, :]
...
@torch.no_grad()
def srcindex_copy_kv(K, srcLoc, Out):
    seq_len = srcLoc.shape[0]
    head_num = K.shape[1]
    head_dim = K.shape[2]
    ...
    _fwd_kernel_srcindex_copy_kv[grid](
        K, srcLoc, Out,
